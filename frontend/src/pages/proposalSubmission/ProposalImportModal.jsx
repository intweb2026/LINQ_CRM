...
export default function ProposalImportModal({ onClose, onImported }) {
  ...
  async function onPickFile(f) {
    ...
    try {
      ...
      setRows(parsedRows);
      // Preview immediately rather than waiting for the button: Import is disabled
      // until a plan exists, so a picked file with a greyed-out Import is a dead end
      // that never says the missing step is Preview. Previewing writes nothing.
      // Passed explicitly because setRows has not landed in state yet.
