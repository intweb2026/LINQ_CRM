...
What it does:
  ...
  3. Creates a GoogleSheetsService pointed at the target spreadsheet
  4. Lists existing tabs in the spreadsheet
  5. Runs build_rows for the bookings module with a small column set
  6. Writes the rows to the sheet via replace_data_chunked
  7. Reports success or the exact error

This does NOT create a SheetSyncTarget record; it exercises the same
code path without touching the database model.
"""
import os
import time
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # Materialise to count; the real runner streams, but for a test
        # we want the count before we decide whether to write.
        rows = list(row_iter)
