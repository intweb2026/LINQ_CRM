...
class NonSalesRoleInvoiceNumberTests(TestCase):
    """
    A role that is neither admin nor `sales` raising a booking from the modal.

    BookEventDetailSerializer.get_fields() marked `invoice_number` read-only for
    every other role, which is right for a RENAME and wrong for a create: the
    number the modal sent was stripped out of validated_data, and BookEvent.save()
    raised ValueError("invoice_number must be pre-generated by the website.") — an
    uncaught 500 on the Save booking button for the whole SpEx team, and for
    Operations, Speaker Sales, Telemarketing, Market Research and Data Mining too.
    ...
    def setUp(self):
        ...
        self.factory = APIRequestFactory()
        self.event = make_event(EVENT_A)
        team = Team.objects.create(name="modal_spex")
        TeamPermission.objects.create(
            team=team, module="bookings",
            can_view=True, can_create=True, can_update=True,
        )
        self.spex = User.objects.create_user(
            username="modal_spex_u", password="x", role="spex",
            email="modal_spex_u@iq-hub.com",
        )
        self.spex.team = team
        self.spex.save()

    def test_spex_user_can_create_a_booking(self):
        req = self.factory.post("/api/invoices/", {
            "invoice_number": "MOD-SPEX", "event_code": EVENT_A,
            "payment_status": "Pending", "booking_code": "Delegate",
            "delegates": [{
                "first_name": "Spex", "last_name": "One",
                "email": "spex.one@acme.test",
            }],
        }, format="json")
        force_authenticate(req, user=self.spex)
        resp = _view({"post": "create"})(req)
        resp.render()

        self.assertEqual(resp.status_code, 201, resp.content)
        inv = BookEvent.objects.get(invoice_number="MOD-SPEX")
        self.assertEqual(inv.delegates.count(), 1)

    def test_spex_user_still_cannot_rename_an_existing_invoice(self):
        """The half of the gate that was always the point of it."""
        inv = BookEvent.objects.create(
            invoice_number="MOD-KEEP", event_code=EVENT_A, payment_status="Pending",
            # Owned by the caller, or rbac_filter scopes the row away and the
            # PATCH is a 404 before the read-only rule is ever reached.
            sales_executive=self.spex,
        )
        req = self.factory.patch(f"/api/invoices/{inv.pk}/",
                                 {"invoice_number": "MOD-RENAMED"}, format="json")
        force_authenticate(req, user=self.spex)
        resp = _view({"patch": "partial_update"})(req, pk=inv.pk)
        resp.render()

        self.assertEqual(resp.status_code, 200, resp.content)
        inv.refresh_from_db()
        self.assertEqual(inv.invoice_number, "MOD-KEEP")
