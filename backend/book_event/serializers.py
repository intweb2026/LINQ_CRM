...
class BookEventDetailSerializer(serializers.ModelSerializer):
    ...
    def get_fields(self):
        ...
        # RENAMING an already-saved invoice is what this gate is for, so it only
        # applies to an UPDATE. On a create the number is the one the modal just
        # typed, and locking it there dropped invoice_number out of
        # validated_data for every other role — BookEvent.save() then raised
        # ValueError("invoice_number must be pre-generated by the website."),
        # which DRF does not translate, so Save booking returned a 500 for SpEx,
        # Operations, Speaker Sales, Telemarketing, Market Research and Data
        # Mining alike. Who may create a booking at all is already decided by
        # crm_permission("bookings"); this had turned into a second, hidden and
        # role-hardcoded copy of that rule.
        if self.instance is not None and not can_edit:
    ...
    def update(self, instance, validated_data):
        ...
        with transaction.atomic():
            ...
            if delegates_data is not None:
                ...
                # The viewset prefetched delegates BEFORE the loop above created
                # and deleted rows, and both remaining reads go through that
                # cache; the count() below is a len() of the stale list, and
                # to_representation() serialises it into the response. Popping
                # sends both back to the database.
