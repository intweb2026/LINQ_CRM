...
class TicketViewSet(PeriodFilterMixin, FilterSpecMixin, BulkUpdateMixin,
    ...
    filter_spec_fields = build_filter_spec_fields(
        Ticket,
        # link_key is a digest nobody can read, let alone type into a filter. It
        # is excluded here, from mass update and from Smart Import for the same
                ...
                if duplicate_mode == "upsert_by_external_id":
                    try:
                        with transaction.atomic():
                            ...
                            # A queryset .update() does not call Model.save(), so
                            # link_key — which save() derives from link_url — has
                            # to be carried here by hand. Without this an upsert
                            # that changes the link leaves the old digest behind
                            # and the repeated-link check silently stops seeing
                            # that ticket.
