...
def scope_tickets(queryset, user):
    ...
    if sees_every_ticket(user):
        return queryset
    # created_by is the reliable half; it is an FK and it is set on every ticket
    # raised through the API.
    own = Q(created_by=user)
    # added_user_text is the secondary half, and it is matched against BOTH forms
    # a real value can take, because the column has carried two conventions:
    # migration 0008 and TicketCreateSerializer write a DISPLAY NAME, while the
    # migrated Zoho rows hold a username. Matching only one form would hand a
    # person rows under one convention and hide them under the other, for no
    # reason they could see.
    from .utils import display_name

    for candidate in {getattr(user, "username", ""), display_name(user) or ""}:
        if candidate:
            own |= Q(added_user_text__iexact=candidate)
